...
def main():
    ...
    try:
        for epoch in range(1, args.epochs+1):
            ...
            if new_lr != old_lr:
                print(f"→ LR reduced: {old_lr:.2e} → {new_lr:.2e}", flush=True)

            if f1 > best_f1:
                best_f1 = f1
                out_path = os.path.join(args.out_dir, "best_model.pth")
                torch.save(model.state_dict(), out_path)
                print(f"→ New best model saved: F1={f1:.3f} → {out_path}", flush=True)

    except KeyboardInterrupt:
        print("⚠️  Training interrupted. Saving model...", flush=True)
