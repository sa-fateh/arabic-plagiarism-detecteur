...
def build_dataset(
    xml_dirs, susp_dirs, src_dirs, out_dir,
    neg_pool_dirs=None, neg_length=50, neg_ratio=2.0,
    ...
    for corpus_id, (xml_dir, susp_dir, src_dir) in enumerate(zip(xml_dirs, susp_dirs, src_dirs)):
        for xml_fp in glob.glob(os.path.join(xml_dir, "*.xml")):
            ...
            base_name = os.path.basename(xml_fp).replace(".xml", ".txt")
            susp_name = f"corpus{corpus_id}_{base_name}"
            susp_txt  = extract_fragment(os.path.join(susp_dir, base_name), 0, 10**7) or ""
            for feat in root.findall("feature"):
                src_ref = feat.get("source_reference")
                src_name = f"corpus{corpus_id}_{src_ref}"
                ann[susp_name].add(src_name)
    ...
    if neg_pool_dirs:
        for neg_dir in neg_pool_dirs:
            pool |= set(glob.glob(os.path.join(neg_dir, "**", "*.txt"), recursive=True))
    pool = list(pool)
    ...
    for susp, grp in df[df.label == 1].groupby("suspicious_reference"):
        susp_base = susp.split("_", 1)[1]
        corpus_id = int(susp.split("_")[0].replace("corpus", ""))
        susp_full = extract_fragment(os.path.join(susp_dirs[corpus_id], susp_base), 0, 10**7) or ""
        q_vec = tfidf.transform([susp_full])
        sims = (doc_vecs @ q_vec.T).toarray().ravel()
        for i in range(len(pool)):
            if f"corpus{corpus_id}_{os.path.basename(pool[i])}" in ann[susp]:
                sims[i] = -1
    ...
    for _, pos in df[df.label == 1].iterrows():
        susp_base = pos.suspicious_reference.split("_", 1)[1]
        corpus_id = int(pos.suspicious_reference.split("_")[0].replace("corpus", ""))
        susp_full = extract_fragment(os.path.join(susp_dirs[corpus_id], susp_base), 0, 10**7) or ""
